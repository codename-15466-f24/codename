...
struct ToneMapProgram {
	ToneMapProgram() {
		...
		,
			...
			"const vec3[6] cs = vec3[6](\n"
			"	vec3(0.0f, 0.0f, 0.0f),\n"
	    	"	vec3(0.002732, 0.001214, 0.061246),\n"
        	"	vec3(0.039546, 0.078187, 0.165132),\n"
        	"	vec3(0.132868, 0.318547, 0.327778),\n"
        	"	vec3(0.386429, 0.539479, 0.304987),\n"
        	"	vec3(0.799103, 0.830770, 0.278894)\n"
    		"	);\n"

			"out vec4 fragColor;\n"
			"void main() {\n"
			"	vec3 color = texelFetch(TEX, ivec2(gl_FragCoord.xy), 0).rgb;\n"
			"	vec3 nbor_colors = fract( color\n"
			"                    + texelFetch(TEX, ivec2(gl_FragCoord.xy) - ivec2(1, 0), 0).rgb\n"
			"                    + texelFetch(TEX, ivec2(gl_FragCoord.xy) - ivec2(0, 1), 0).rgb);\n"
			
			// if (gl_FragCoord.x > 10. && gl_FragCoord.y > 10 
			//  && gl_FragCoord.x < textureSize(TEX) - 10 && gl_FragCoord.y < textureSize(TEX)) {
			// 	for (int i = -10; i < 10; i++) {
			// 		if ();
			// 	};
			// }
			...
			"	float min_dist = 1./0.;\n"
			"	for (int c = 0; c < 6; c++) {\n"
			"		float dist = length(color - cs[c]);\n"
			"		if (dist <= min_dist) {\n"
			"			neighbor = c;\n"
			"			min_dist = dist;\n"
			"		}\n"
			"	}\n"
			"   vec3 final_col = cs[neighbor];\n" //mix(cs[max(neighbor-1, 0)], cs[neighbor], e*(1.-min_dist));\n"
